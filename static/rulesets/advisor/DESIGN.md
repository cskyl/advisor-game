...
- **Funding as a budget**: startup 100; monthly burn = `students*stipend(2) + compute − 2` (institutional support). Grants are now a **3-tier random with rep-tilted odds**: a rare jackpot `clip(0.08 + rep/500, 0.05, 0.22)` for `randi(50)+70+rep/3` funding (and +2 rep), a modest grant `clip(0.32 + rep/400, 0.22, 0.55)` for `randi(20)+15+rep/5`, and the rest is triaged (−2 morale). Most attempts fail; the big ones are real moments. Windfalls and networking gifts also less frequent but bigger.
...
Near-optimal **cautious** play: ~99% survive, ~always tenure, **~98% dream**, avg ~1.5 jackpot grants/game. **Aggressive multi-recruit growth** (sim buffer 80): ~47% survive — the variance-heavy economy bites over-extended labs. Smart players watch the runway HUD and pace recruiting, landing between. The dream is forgiving; the cost is real if you over-expand. Knobs: `Init` (`rule.*`, starting values), the production/acceptance expressions, the rep-scaled grant weights, venue `conf.*` in `ConferenceOpens`, `status.yaml` drains. **The sim is a separate model — mirror any number change between it and the YAML, then re-run it.**
